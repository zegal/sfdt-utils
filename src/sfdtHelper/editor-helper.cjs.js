...
var HelperMethods = /** @class */ (function() {
  ...
  HelperMethods.indexOfAny = function(text, wordSplitCharacter) {
    ...
    for (var j = 0; j < wordSplitCharacter.length; j++) {
      ...
      if (temp !== -1 && ej2Base.isNullOrUndefined(index)) {
        index = temp;
      } else if (temp !== -1 && temp < index) {
        index = temp;
      }
    }
    return ej2Base.isNullOrUndefined(index) ? -1 : index;
  };
  /**
  ...
  //     return !ej2Base.isNullOrUndefined(inline.fieldEnd);
  //   } else if (inline instanceof FieldElementBox && inline.fieldType === 2) {
  //     return (
  //       !ej2Base.isNullOrUndefined(inline.fieldBegin) &&
  //       !ej2Base.isNullOrUndefined(inline.fieldEnd)
  //     );
  //   } else {
  //     return !ej2Base.isNullOrUndefined(inline.fieldBegin);
  //   }
  // };
  /**
  ...
  HelperMethods.removeSpace = function(text) {
    if (!ej2Base.isNullOrUndefined(text) && text.length !== 0) {
   ...
   */
  HelperMethods.writeCharacterFormat = function(
    characterFormat,
    isInline,
    format
  ) {
    characterFormat.bold = isInline ? format.bold : format.getValue("bold");
    characterFormat.italic = isInline
      ? format.italic
      : format.getValue("italic");
    characterFormat.fontSize = isInline
      ? format.fontSize
      : format.getValue("fontSize");
    characterFormat.fontFamily = isInline
      ? format.fontFamily
      : format.getValue("fontFamily");
    characterFormat.underline = isInline
      ? format.underline
      : format.getValue("underline");
    characterFormat.strikethrough = isInline
      ? format.strikethrough
      : format.getValue("strikethrough");
    characterFormat.baselineAlignment = isInline
      ? format.baselineAlignment
      : format.getValue("baselineAlignment");
    characterFormat.highlightColor = isInline
      ? format.highlightColor
      : format.getValue("highlightColor");
    characterFormat.fontColor = isInline
      ? format.fontColor
      : format.getValue("fontColor");
    characterFormat.styleName = !ej2Base.isNullOrUndefined(format.baseCharStyle)
      ? format.baseCharStyle.name
      : undefined;
    ...
    characterFormat.boldBidi = isInline
      ? format.boldBidi
      : format.getValue("boldBidi");
    characterFormat.italicBidi = isInline
      ? format.italicBidi
      : format.getValue("italicBidi");
    characterFormat.fontSizeBidi = isInline
      ? format.fontSizeBidi
      : format.getValue("fontSizeBidi");
    characterFormat.fontFamilyBidi = isInline
      ? format.fontFamilyBidi
      : format.getValue("fontFamilyBidi");
  };
  ...
  HelperMethods.ReverseString = function(text) {
    if (!ej2Base.isNullOrUndefined(text) && text !== "") {
      // return a new array
  ...
  HelperMethods.formatClippedString = function(base64ImageString) {
    ...
    if (this.startsWith(base64ImageString, "data:image/bmp;base64,")) {
      extension = ".bmp";
      formatClippedString = base64ImageString.replace(
